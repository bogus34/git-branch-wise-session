 ...
 */
export default class VscodeSessionManagerDelegate
    implements SessionManagerDelegate
{
    ...
    constructor(
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand(
                "git-branch-wise-session.saveSession",
                async () => {
                    this.onSessionSaveRequestedListener(
                        await this.getCurrentSession()
                    );
                }
            ),
            vscode.commands.registerCommand(
                "git-branch-wise-session.restoreSession",
                () => {
                    this.onSessionRestoreRequestedListener(this.nameOfSession);
                }
            ),
            vscode.commands.registerCommand(
                "git-branch-wise-session.clearSession",
                () => {
                    this.onSessionClearRequestedListener(this.nameOfSession);
                }
            ),
            vscode.commands.registerCommand(
                "git-branch-wise-session.clearAllSessions",
                () => {
    ...
    async getCurrentSession(): Promise<Session> {
        ...
        const equals = (lhs: vscode.TextEditor, rhs: vscode.TextEditor) => {
            return (
                lhs.document.uri === rhs.document.uri &&
    ...
    private intoSession(vscodeEditors: vscode.TextEditor[]): Session {
        ...
        const editors = vscodeEditors.map(
            (editor: vscode.TextEditor, order: number) => {
                return {
    ...
    setOnSessionRestoreRequestedListener(
        listener: OnSessionRestoreRequestedListener
    ) {
        this.onSessionRestoreRequestedListener = listener;
    }

    setOnSessionSaveRequestedListener(
        listener: OnSessionSaveRequestedListener
    ) {
    ...
    setOnSessionClearRequestedListener(
        listener: OnSessionClearRequestedListener
    ) {
        this.onSessionClearRequestedListener = listener;
    }

    setOnSessionAllClearRequestedListener(
        listener: OnSessionAllClearRequestedListener
    ) {
    ...
    async restore(session: Session): Promise<void> {
        ...
        for (const editor of session.editors) {
            ...
            if (editor.column >= 0) {
                options.viewColumn = editor.column;
            }
